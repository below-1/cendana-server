...
import * as toolTransServices from './service'
...
export async function post(request: PostRequest, reply: Reply) {
  ...
  const trans = await toolTransServices.create(payload)
  reply.send(trans)
}
...
export async function put(request: PutRequest, reply: Reply) {
  ...
  const { body: payload } = request
  const trans = await toolTransServices.update(id, payload)
  reply.send(trans)
}

export async function remove(request: DelRequest, reply: Reply) {
  const { id } = request.params
  const trans = await toolTransServices.remove(id)
}

export async function getOne(request: DelRequest, reply: Reply) {
  const { id } = request.params
  const trans = await toolTransServices.findOne(id)
  if (!trans) {
